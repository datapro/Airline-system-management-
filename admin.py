...
def search():
  ...
  if flight_number:
    ...
    if result:
      ...
      flight_info=f"flight{result[0]} from {result[1]} to {result[2]} deaprt at {result[3]} and arrive at {result[4]}."
      text_to_speech(flight_info)
    else:
...
lbl=Label(admin, text="Admin Panel", font=("sanseri",50,"bold"), bg="#5e546e", fg="#fff", underline=6)
lbl.place(x=20, y=0)
lbluser=Label(admin, text="click the button below to access user dashboard", font=("sanseri",12), bg="#5e546e", fg="red")
lbluser.place(x=20, y=80)
btnuser=Button(admin, text="click me", bg="gray", fg="white", border=0, command=user)
btnuser.place(x=200,y=110)
...
btndelete=Button(right_frame,text="Delete Flight", font=("sanserif", 14,"bold"), fg="black", bg="red", command=delete)
